...
post :create, :user => @attr
flash[:success].should =˜ /welcome to the sample app(sukses)/i
end
...
before(:each) do
admin = Factory(:user, :email => "admin@exam
test_sign_in(admin)
User.should_receive(:find).with(@user).and_r
@user.should_receive(:destroy).and_return(@u
end
